...
if [[ "${PHASE2_TEST_FIXTURES:-0}" == "1" ]]; then
  INPUT_FILES=(tests/fixtures/nginx.jsonl tests/fixtures/services.jsonl)
else
  INPUT_FILES=(data/nginx/nginx_access.log data/service_logs/*.log)
fi
